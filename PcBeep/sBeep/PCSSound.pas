...
const Freq2Max: array [0..11] of single = (8372, 8869.6, 9396.8, 9956, 10548, 11175.2, 11840, 12544, 13289.6, 14080, 14916.8, 15804);
var aFreq:Single; RegExp: TRegExpr; note_ind,note_oct,note_offset:integer; sm:string; c:char; pause:boolean; es:integer;
begin
RegExp := TRegExpr.Create; RegExp.Expression := '([A-H]|[a-h]|P|p)(is|IS|iS|Is|es|ES|eS|Es|)([0-9]|)';
if RegExp.Exec(StrNote) then sm := RegExp.Match[1]; c:=sm[1]; pause := false;
case (c) of
'C','c': note_ind := 0;
'D','d': note_ind := 2;
'E','e': note_ind := 4;
...
'H','h': note_ind := 11;
'P','p': pause := true; end;
if pause then begin NoSound; sleep(delay); end else
begin es := 0;
if (UpperCase(RegExp.Match[2])=UpperCase('is')) then note_offset := 1 else
if (UpperCase(RegExp.Match[2])=UpperCase('es')) then begin es := 128; note_offset := -1; end else note_offset := 0;
if (length(RegExp.Match[3])=1) then note_oct := strtoint(RegExp.Match[3]) else note_oct := 0;
aFreq:=Freq2Max[note_ind+note_offset] / (2 shl (8-note_oct));
Sound(trunc(aFreq),delay); end;
end;
